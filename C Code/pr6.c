...
int main() {
    ...
    scanf("%d", &n);
    int process_id[n], burst_time[n], priority[n], arrival_time[n];
    ...
    for (int i = 0; i < n - 1; i++) {
        for (int j = i + 1; j < n; j++) {
            if (arrival_time[i] > arrival_time[j] ||
               (arrival_time[i] == arrival_time[j] && priority[i] > priority[j])) {
                int temp;
                temp = arrival_time[i];
    ...
    for (int i = 0; i < n; i++) {
        printf("P%d\t%d\t%d\t%d\t%d\t%d\t%d\n", 
               process_id[i], arrival_time[i], burst_time[i], priority[i], 
